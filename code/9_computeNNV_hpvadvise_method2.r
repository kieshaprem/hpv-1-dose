...
for(COUNTRY in c('India','Nigeria','Uganda','Vietnam','Canada')) load(paste0('output/prime/impact_hpvadvise',COUNTRY,'.rdata'))
...
for(COUNTRY in c('India','Nigeria','Uganda','Vietnam','Canada'))
{
  ...
  include = data.frame(year =2020:2209, include = c(rep(1, length(2020:2120)),rep(0,length(2121:2209))))
  ...
  for(R in 1:length(IMPACT[[1]]))
  {
    ...
    for(SCEN in 1:4)
    {
      ...
      IMPACT[[SCEN]][[R]]$include = include$include[match(IMPACT[[SCEN]][[R]]$year,include$year)]
      ...
      TEMP = aggregate(IMPACT[[SCEN]][[R]]$cases_averted*IMPACT[[SCEN]][[R]]$include,by = list(IMPACT[[SCEN]][[R]]$iso3c),sum)
      total_cancers_averted[,SCEN] = TEMP[match(rownames(total_cancers_averted),TEMP[,1]),2]
      rm(TEMP)
      
      TEMP = aggregate(IMPACT[[SCEN]][[R]]$cases_averted_dish*IMPACT[[SCEN]][[R]]$include,by = list(IMPACT[[SCEN]][[R]]$iso3c),sum)
  ...
  # str(no_to_vaccinate)
  no_to_vaccinate = no_to_vaccinate[c(1,4,5)]
  no_to_vaccinate_dish = no_to_vaccinate_dish[c(1,4,5)]
  
  save(no_to_vaccinate,file=paste0('output/nnv_',model,'_M2.rdata'))
  save(no_to_vaccinate_dish,file=paste0('output/nnv_dish_',model,'_M2.rdata'))
}