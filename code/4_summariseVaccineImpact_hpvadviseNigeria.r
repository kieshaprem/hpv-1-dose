...
impact_hpvadviseNigeria = list(protect_life=list(),
                  protect_20y=list(),
                  protect_30y=list(),
                  protect_life80take = list())


summariseVaccineImpact = function(d,RUN,MODEL='hpvadviseNigeria')
{
  ...
  names(aggregated_data) = c('prevac_cases_all','prevac_deaths_all','prevac_dalys_all',
                             'cases_averted','deaths_averted','dalys_averted','cases_averted_disc','deaths_averted_disc','dalys_averted_disc','postvac_cases_all','postvac_deaths_all','postvac_dalys_all','total_vaccinated')
...
load(file = 'output/prime/vaccine_impact_hpvadviseNigeria_life.rdata')
vaccine_impact_hpvadviseNigeria = list(strategy_life = strategy_life)
scenario = 'strategy_life'
for(run in 1:5) 
{
  d = data.frame(vaccine_impact_hpvadviseNigeria[[scenario]][[run]],total_vaccinated = vaccine_impact_0$total_vaccinated)#cbind(vaccine_impact_0,vaccine_impact_hpvadviseNigeria[[scenario]][[run]])
  d = d[d$calendar_year > 2019,] #& d$calendar_year < 2081,]
  
  impact_hpvadviseNigeria$protect_life[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseNigeria,run,strategy_life)


load(file = 'output/prime/vaccine_impact_hpvadviseNigeria_20y.rdata')
vaccine_impact_hpvadviseNigeria = list(strategy_20y = strategy_20y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseNigeria$protect_20y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseNigeria,run,strategy_20y)


load(file = 'output/prime/vaccine_impact_hpvadviseNigeria_30y.rdata')
vaccine_impact_hpvadviseNigeria = list(strategy_30y = strategy_30y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseNigeria$protect_30y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseNigeria,run,strategy_30y)


load(file = 'output/prime/vaccine_impact_hpvadviseNigeria_life80take.rdata')
vaccine_impact_hpvadviseNigeria = list(strategy_life80take = strategy_life80take)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseNigeria$protect_life80take[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseNigeria,run,strategy_life80take)

save(impact_hpvadviseNigeria,file = 'output/prime/impact_hpvadviseNigeria.rdata')
