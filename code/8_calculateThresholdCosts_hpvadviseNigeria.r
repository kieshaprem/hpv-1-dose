load('output/costs/costs.rdata')
load('output/prime/impact_hpvadviseNigeria.rdata')

IMPACT = impact_hpvadviseNigeria
model = 'hpvadviseNigeria'
...
dataall = matrix(NA, nrow = length(countries),ncol = length(IMPACT))
rownames(dataall) = countries
colnames(dataall) = names(IMPACT)
...
for(R in 1:length(IMPACT[[1]]))
{
  ...
  for(SCEN in 1:4)
  {
    ...
    IMPACT[[SCEN]][[R]]$dalys_averted_gdp = IMPACT[[SCEN]][[R]]$dalys_averted*IMPACT[[SCEN]][[R]]$gdp_per_capita 
    IMPACT[[SCEN]][[R]]$dalys_averted_gdp_dish = IMPACT[[SCEN]][[R]]$dalys_averted_disc*IMPACT[[SCEN]][[R]]$gdp_per_capita 
...
save(threshold,file=paste0('output/costs/threshold_',model,'.rdata'))
