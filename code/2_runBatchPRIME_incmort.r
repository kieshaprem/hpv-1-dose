...
coverage = c(array(0.8,length(2020:2100)))
batch_file = data.frame(country_code = rep(as.character(unique(data.pop$country_code)),each = length(coverage)), 
                        year = rep(2020:2100,length(as.character(unique(data.pop$country_code)))),
                        age_first = 10, 
                        age_last = 10,
...
batch_input <- batch_file %>% dplyr::filter(year >= 2020)

# Save and read batch input file.
write.table(batch_input, "output/batch_input_inc.csv", sep = ",", col.names = T, row.names = F)
batch_file <- fread("output/batch_input_inc.csv", sep = ",", header= T)
...
{  #iso3c_code = "AFG"
  ...
  if (a>0) {
    ...
    write.table (batch_input, "output/temp_inc.csv", sep=",", col.names = T, row.names=F)
    batch_input = fread ("output/temp_inc.csv", sep = ",", header= TRUE)
    #####
    ...
    if ((nrow(country_impact)) > 0) {
      ...
      { ##
        ...
        if (unique(temp_impact$year_vac) > as.numeric(early_cohort - 1)) 
        {
          ...
          temp_impact$postvac_mort_cecx <- (post_vac$mort.cecx)# + (pre_vac$mort.cecx * ((1 - 1) / 1))) * pre_vac$cohort_size  # direct only #prec_all_prime - ((prec_all_prime - postc_all_prime) * (s + temp_impact$PHE_multiplier))
          ...
          temp_impact$postvac_yldyll <- ((post_vac$disability + post_vac$lifey))# + ((pre_vac$disability + pre_vac$lifey) * ((1 - 1) / 1))) * pre_vac$cohort_size
...
cecx_incmort <- rbindlist(temp_list)
save(cecx_incmort, file = 'output/prime/vaccine_impact_inc_mort.rdata')
