...
png('output/plots/fig_deathaverted_dish.png',width = 20,height = 23.5,units = 'cm',res = 700)
if(1)
{
  grid.newpage()
  ...
  for (REG in 1:4) 
  {
    ...
    grid.text("Cervical cancers deaths averted",x=unit(-4.25,'lines'),rot=90,gp=gpar(fontface='plain',fontsize=9))
    grid.text(expression(paste(""%*%"10"^"3")),x=unit(-3,'lines'),rot=90,gp=gpar(fontface='plain',fontsize=9))
    
    xvals = seq(2020,2209,1)
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 2,VAR = 'deaths_averted_disc',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 3,VAR = 'deaths_averted_disc',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
      grid.lines(x=xvals,y=yvals,default.units = 'native',gp = gpar(lwd=1,col=COLS[model],lty = LTY[model]))
    }
    grid.rect(gp = gpar(fill=NA,lwd=1.5))
    popViewport()
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 4,VAR = 'deaths_averted_disc',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
      grid.lines(x=xvals,y=yvals,default.units = 'native',gp = gpar(lwd=1,col=COLS[model],lty = LTY[model]))
    }
    grid.rect(gp = gpar(fill=NA,lwd=1.5))
    popViewport()
...
png('output/plots/fig_deathsaverted.png',width = 20,height = 23.5,units = 'cm',res = 700)
if(1)
{
  grid.newpage()
  pushViewport(plotViewport(c(1,3,3,0)))
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 2,VAR = 'deaths_averted',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
      grid.lines(x=xvals,y=yvals,default.units = 'native',gp = gpar(lwd=1,col=COLS[model],lty = LTY[model]))
    }
    grid.rect(gp = gpar(fill=NA,lwd=1.5))
    popViewport()
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 3,VAR = 'deaths_averted',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
      grid.lines(x=xvals,y=yvals,default.units = 'native',gp = gpar(lwd=1,col=COLS[model],lty = LTY[model]))
    }
    grid.rect(gp = gpar(fill=NA,lwd=1.5))
    popViewport()
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:11) 
    {
      yvals = getYvalues(protect = protect,PROTECT = 4,VAR = 'deaths_averted',REGIONS = REGIONNAMES[[REG]],SMOOTH = TRUE)[,model]
      grid.lines(x=xvals,y=yvals,default.units = 'native',gp = gpar(lwd=1,col=COLS[model],lty = LTY[model]))
    }
    grid.rect(gp = gpar(fill=NA,lwd=1.5))
    popViewport()
