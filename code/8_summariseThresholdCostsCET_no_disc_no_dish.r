...
for(index in 1:11)#  if(index==1)
{
  ...
  for(loc in 1:4) 
  {
    ...
    thresholdDATA = as.data.frame(rbindlist(threshold_costs[[index]]$threshold_costs)) #threshold_costs[[index]]$threshold_costs_disc_dish[[1]] #
...
save(thresholdAcet,file = 'output/costs/thresholdAcet_undis.rdata')
save(thresholdBcet,file = 'output/costs/thresholdBcet_undis.rdata')
save(thresholdCcet,file = 'output/costs/thresholdCcet_undis.rdata')
save(thresholdDcet,file = 'output/costs/thresholdDcet_undis.rdata')
save(thresholdEcet,file = 'output/costs/thresholdEcet_undis.rdata')
save(thresholdFcet,file = 'output/costs/thresholdFcet_undis.rdata')
