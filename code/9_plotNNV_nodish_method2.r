...
10^4.75
YMAX = list(10^5,10^5,10^5,10^5)#28000,28000,28000,28000)
...
png('output/plots/fig_nnvM2.png',width = 38,height = 10.2,units = 'cm',res = 700)
if(1)
{
  grid.newpage()
  ...
  for (REG in 1:4) 
  {
    ...
    for(model in 1:9) 
    {
      if(model<10) grid.lines(x=0.6+0.1*(model-1),y=log10(c(nnvA[[model]][3,REG],nnvA[[model]][4,REG])),default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=0.6+0.1*(model-1),y=log10(c(nnvA[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
      
      if(model<10) grid.lines(x=1.6+0.1*(model-1),y=log10(c(nnvB[[model]][3,REG],nnvB[[model]][4,REG])),default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=1.6+0.1*(model-1),y=log10(c(nnvB[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
      
      if(model<10) grid.lines(x=2.6+0.1*(model-1),y=log10(c(nnvC[[model]][3,REG],ifelse(nnvC[[model]][4,REG]>YMAX[[REG]],YMAX[[REG]],nnvC[[model]][4,REG]))),
                             default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=2.6+0.1*(model-1),y=log10(c(nnvC[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
      # if(nnvC[[model]][4,REG]>YMAX[[REG]]) 
      if(model<10) grid.lines(x=3.6+0.1*(model-1),y=log10(c(nnvD[[model]][3,REG],ifelse(nnvD[[model]][4,REG]>YMAX[[REG]],YMAX[[REG]],nnvD[[model]][4,REG]))),
                             default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=3.6+0.1*(model-1),y=log10(c(nnvD[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
      
      if(model<10) grid.lines(x=4.6+0.1*(model-1),y=log10(c(nnvE[[model]][3,REG],ifelse(nnvE[[model]][4,REG]>YMAX[[REG]],YMAX[[REG]],nnvE[[model]][4,REG]))),
                              default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=4.6+0.1*(model-1),y=log10(c(nnvE[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
      # if(nnvC[[model]][4,REG]>YMAX[[REG]]) 
      if(model<10) grid.lines(x=5.6+0.1*(model-1),y=log10(c(nnvF[[model]][3,REG],ifelse(nnvF[[model]][4,REG]>YMAX[[REG]],YMAX[[REG]],nnvF[[model]][4,REG]))),
                              default.units = 'native',gp=gpar(col=COLS[model]))
      grid.points(x=5.6+0.1*(model-1),y=log10(c(nnvF[[model]][1,REG])),default.units = 'native',pch=16,gp=gpar(col=COLS[model],cex=0.3))
