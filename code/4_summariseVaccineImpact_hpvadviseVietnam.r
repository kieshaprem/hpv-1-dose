...
impact_hpvadviseVietnam = list(protect_life=list(),
                              protect_20y=list(),
                              protect_30y=list(),
                              protect_life80take = list())


summariseVaccineImpact = function(d,RUN,MODEL='hpvadviseVietnam')
{
...
load(file = 'output/prime/vaccine_impact_hpvadviseVietnam_life.rdata')
vaccine_impact_hpvadviseVietnam = list(strategy_life = strategy_life)
scenario = 'strategy_life'
for(run in 1:5) 
{
  d = data.frame(vaccine_impact_hpvadviseVietnam[[scenario]][[run]],total_vaccinated = vaccine_impact_0$total_vaccinated)#cbind(vaccine_impact_0,vaccine_impact_hpvadviseVietnam[[scenario]][[run]])
  d = d[d$calendar_year > 2019,] #& d$calendar_year < 2081,]
  
  impact_hpvadviseVietnam$protect_life[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseVietnam,run,strategy_life)


load(file = 'output/prime/vaccine_impact_hpvadviseVietnam_20y.rdata')
vaccine_impact_hpvadviseVietnam = list(strategy_20y = strategy_20y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseVietnam$protect_20y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseVietnam,run,strategy_20y)


load(file = 'output/prime/vaccine_impact_hpvadviseVietnam_30y.rdata')
vaccine_impact_hpvadviseVietnam = list(strategy_30y = strategy_30y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseVietnam$protect_30y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseVietnam,run,strategy_30y)


load(file = 'output/prime/vaccine_impact_hpvadviseVietnam_life80take.rdata')
vaccine_impact_hpvadviseVietnam = list(strategy_life80take = strategy_life80take)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseVietnam$protect_life80take[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseVietnam,run,strategy_life80take)

save(impact_hpvadviseVietnam,file = 'output/prime/impact_hpvadviseVietnam.rdata')
