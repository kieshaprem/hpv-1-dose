...
impact_hpvadviseUganda = list(protect_life=list(),
                  protect_20y=list(),
                  protect_30y=list(),
                  protect_life80take = list())


summariseVaccineImpact = function(d,RUN,MODEL='hpvadviseUganda')
{
...
load(file = 'output/prime/vaccine_impact_hpvadviseUganda_life.rdata')
vaccine_impact_hpvadviseUganda = list(strategy_life = strategy_life)
scenario = 'strategy_life'
for(run in 1:5) 
{
  d = data.frame(vaccine_impact_hpvadviseUganda[[scenario]][[run]],total_vaccinated = vaccine_impact_0$total_vaccinated)#cbind(vaccine_impact_0,vaccine_impact_hpvadviseUganda[[scenario]][[run]])
  d = d[d$calendar_year > 2019,] #& d$calendar_year < 2081,]
  
  impact_hpvadviseUganda$protect_life[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseUganda,run,strategy_life)


load(file = 'output/prime/vaccine_impact_hpvadviseUganda_20y.rdata')
vaccine_impact_hpvadviseUganda = list(strategy_20y = strategy_20y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseUganda$protect_20y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseUganda,run,strategy_20y)


load(file = 'output/prime/vaccine_impact_hpvadviseUganda_30y.rdata')
vaccine_impact_hpvadviseUganda = list(strategy_30y = strategy_30y)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseUganda$protect_30y[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseUganda,run,strategy_30y)


load(file = 'output/prime/vaccine_impact_hpvadviseUganda_life80take.rdata')
vaccine_impact_hpvadviseUganda = list(strategy_life80take = strategy_life80take)
...
for(run in 1:5) 
{
  ...
  impact_hpvadviseUganda$protect_life80take[[run]] = summariseVaccineImpact(d=d,RUN = run)
  rm(d)
  print(run)
}
rm(vaccine_impact_hpvadviseUganda,run,strategy_life80take)

save(impact_hpvadviseUganda,file = 'output/prime/impact_hpvadviseUganda.rdata')
